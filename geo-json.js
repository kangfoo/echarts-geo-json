...
const keys = [
  '3387f2aae76ebd4f915de5ade19634dc',
  '806178332e93277d48d196c6981d3598',
  '35ea21e11d1a80d9ef41014a1b8513b4',
  '27b4b0aeb8dd4c32614f13d3e3fd866f',
  '09ee92217380c74c07119fbc31191078',
  '188237b34dde0db2acdb536db5763df9',
  '5f9f8a5b20757d48f26cce0125f3f24d',
  '93495dcc4cc8085550fff629c000616e',
  'fb206e2bb6c03d6d021de271c64b3d3d',
];

let serachFromAMap = async (adcode, polyline) => {
  try {
    let key = keys[Math.floor(Math.random() * keys.length)];
    let url = `http://restapi.amap.com/v3/config/district?key=${key}&subdistrict=${
      polyline ? 0 : 1
    }&extensions=${polyline ? 'all' : 'base'}&keywords=${adcode}`;
    ...
    while (true) {
      if (data.infocode !== '10000') {
        ...
        url = `http://restapi.amap.com/v3/config/district?key=${key}&subdistrict=${
          polyline ? 0 : 1
        }&extensions=${polyline ? 'all' : 'base'}&keywords=${adcode}`;
...
let parseToFeature = district => {
  try {
    ...
    let coordinates = [];
    let coordinateGroups = polylines.map(polyline =>
      polyline.split(';').map(pointStr => {
    ...
    feature.geometry.type =
      coordinateGroups.length > 1 ? 'MultiPolygon' : 'Polygon';
